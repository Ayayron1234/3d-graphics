...
//
//struct ScopedStyle {
//	ScopedStyle(ImGuiStyleVar var, const ImVec2& value) {
...
//	while(window.isOpen()) {
//		// camera
//		if (Mouse::isButtonDown(Mouse::MIDDLE) && !isModPressed(KeyMod::LSHIFT))
//			camController.tilt(vec2(Mouse::motion()) * vec2(.007f, -.01f));
//		if (Mouse::isButtonDown(Mouse::MIDDLE) && isModPressed(KeyMod::LSHIFT))
//			camController.move(vec2(Mouse::motion()) * vec2(.002f, -.002f) * camController.getZoom());
//		if (keyPressed(Key::SPACE)) {
//			camController.setPivot({ 0, 0, 0 });
//			camController.setPosition({ 0, 4, 5 });
//		}
//		camController.zoom(Mouse::wheel());
//
...
//
//
//		if (Mouse::isButtonDown() && Mouse::didMove()) {
//			hitObjects(debug::camera->castRay(window.viewport().windowToNdc(Mouse::position())), objects, rays);
//		}
//		for (auto& ray : rays)
